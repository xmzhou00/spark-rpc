...
import com.xmzhou.rpc.{RpcAddress, RpcEndpoint, ThreadSafeRpcEndpoint}
...
private[netty] class Inbox(
  ...
  protected def onDrop(message: InboxMessage): Unit = {
    log.debug(s"Drop $message because $endpointRef is stopped")
  }
  ...
  private def safelyCall(endpoint: RpcEndpoint)(action: => Unit): Unit = {
    def dealWithFatalError(fatal: Throwable): Unit = {
      ...
      log.error(s"An error happened while processing message in the inbox for $endpointRef", fatal)
      throw fatal
    }
