...
import com.xmzhou.RpcException
import com.xmzhou.rpc.{RpcAddress, RpcEnvStoppedException}
...
private[netty] case class OneWayOutboxMessage(content: ByteBuffer) extends OutboxMessage {
  ...
  private val logger = LoggerFactory.getLogger(classOf[OneWayOutboxMessage])
  ...
  override def onFailure(e: Throwable): Unit = {
    e match {
      case e1: RpcEnvStoppedException => logger.debug(e1.getMessage)
      case e1: Throwable => logger.warn(s"Failed to send one-way RPC.", e1)
    }
  }
...
private[netty] case class RpcOutboxMessage(
  ...
  def onTimeout(): Unit = {
    if (client != null) {
      ...
      logger.error("Ask timeout before connecting successfully")
    }
  }
